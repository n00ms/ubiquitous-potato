...
def board(ship,prob): #one board, three game modes

    board = []
    row = 1
    probability = prob #probability differs depending on game mode
        ...
        while column <= 60:
            ...
            if ship == 0:
                ...
                if surprise >= 1 and surprise <= 4 and surprise in rowBoard: 
                    column = column #if duplicate (a ship represented by that number already exists), skip
                else:
                    if surprise == 0:
                        rowBoard.append(surprise)
                        column += 1
                    else:
                        if surprise >= 1 and surprise <= 4: #ship represented by numbers 1 to 4
                            for i in range(5):
                                rowBoard.append(surprise)
                        column += 5
                        ship -= 1
...
####################display time########################
    ...
    shipCounter = 0 
    while booms <= 15: 
        if shipCounter == 5: #exit loop once 5 ships bombed
            display()
            countRow = 1
                    ...
                    if bombed == 5:
                        ...
                        if bombed == 6:
                            ...
                            if bombed >= 1 and bombed <= 4: 
                                ...
                                print ("You've sunk my battleship!")
                                shipIndex = []
                                for n, i in enumerate(board[userRow-1]): #record unmasked ship in board, unmask in display
                                    if i == bombed:
